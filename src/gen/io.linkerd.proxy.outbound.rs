...
pub mod outbound_policies_client {
    ...
    impl OutboundPoliciesClient<tonic::transport::Channel> {
...
pub mod outbound_policies_server {
    ...
    #[async_trait]
    pub trait OutboundPolicies: std::marker::Send + std::marker::Sync + 'static {
        async fn get(
            &self,
    ...
    #[derive(Debug)]
    pub struct OutboundPoliciesServer<T> {
        inner: Arc<T>,
    ...
    impl<T> OutboundPoliciesServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T> Clone for OutboundPoliciesServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "io.linkerd.proxy.outbound.OutboundPolicies";
    impl<T> tonic::server::NamedService for OutboundPoliciesServer<T> {
