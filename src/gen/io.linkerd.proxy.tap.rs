...
#[derive(Clone, PartialEq, ::prost::Message)]
pub struct WatchResposne {
    #[prost(oneof = "watch_resposne::Kind", tags = "1")]
    pub kind: ::core::option::Option<watch_resposne::Kind>,
}
/// Nested message and enum types in `WatchResposne`.
pub mod watch_resposne {
    #[derive(Clone, PartialEq, ::prost::Oneof)]
    pub enum Kind {
        #[prost(bytes, tag = "1")]
        Spans(::prost::alloc::vec::Vec<u8>),
    }
}
#[derive(Clone, PartialEq, ::prost::Message)]
...
pub struct ObserveTraceRequest {
    ...
    pub matches: ::prost::alloc::vec::Vec<TraceMatch>,
}
#[derive(Clone, PartialEq, ::prost::Message)]
...
pub mod instrument_client {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + std::marker::Send,
    {
        ...
        pub async fn watch(
            ...
            tonic::Response<tonic::codec::Streaming<super::WatchResposne>>,
            tonic::Status,
        > {
            self.inner
                .ready()
            ...
            let codec = tonic::codec::ProstCodec::default();
            let path = http::uri::PathAndQuery::from_static(
                "/io.linkerd.proxy.tap.Instrument/Watch",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(GrpcMethod::new("io.linkerd.proxy.tap.Instrument", "Watch"));
            self.inner.server_streaming(req, path, codec).await
        }
    }
}
...
pub mod instrument_server {
    ...
    pub trait Instrument: std::marker::Send + std::marker::Sync + 'static {
        ...
        type WatchStream: tonic::codegen::tokio_stream::Stream<
                Item = std::result::Result<super::WatchResposne, tonic::Status>,
            >
            + std::marker::Send
            + 'static;
        async fn watch(
        ...
        B::Error: Into<StdError> + std::marker::Send + 'static,
    {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/io.linkerd.proxy.tap.Instrument/Watch" => {
                    #[allow(non_camel_case_types)]
                    struct WatchSvc<T: Instrument>(pub Arc<T>);
                    impl<
                        T: Instrument,
                    > tonic::server::ServerStreamingService<super::WatchRequest>
                    for WatchSvc<T> {
                        type Response = super::WatchResposne;
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as Instrument>::watch(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    /// Generated gRPC service name
    pub const SERVICE_NAME: &str = "io.linkerd.proxy.tap.Instrument";
